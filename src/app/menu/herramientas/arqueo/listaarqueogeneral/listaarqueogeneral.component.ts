...
import { ListaarqueogeneralDataSource, ListaarqueogeneralItem } from './listaarqueogeneral-datasource';
...
@Component({
  selector: 'app-listaarqueogeneral',
  templateUrl: './listaarqueogeneral.component.html',
  styleUrls: ['./listaarqueogeneral.component.css']
})
export class ListaarqueogeneralComponent implements AfterViewInit, OnInit {
  @ViewChild(MatPaginator, {static: false}) paginator: MatPaginator;
  @ViewChild(MatSort, {static: false}) sort: MatSort;
  @ViewChild(MatTable, {static: false}) table: MatTable<ListaarqueogeneralItem>;
  toppingsOficinas = new FormControl();
  dataSource: ListaarqueogeneralDataSource;
  matcher = new MyErrorStateMatcher();

  /** Columns displayed in the table. Columns IDs can be added, removed, or reordered. */
  displayedColumns = ['unidadNegocio', 'valorDeuda', 'pendiente', 'exitoso'];
  datosP=[];
  datosPA=[];
  oficinas=[];
  pickerInicio: string;
  ...
  ngOnInit() {    
    ...
    //-------------------------------------------------------------------------------------------//
    this.dataSource = new ListaarqueogeneralDataSource();
    this.recobrosService.findHerramientasArqueoGeneral().subscribe(data => {      
      for(var _i=0 ; _i<data.length; _i++){
        ...
        this.datosP= [ ...this.datosP, { unidadNegocio: (data[_i])[0], valorDeuda: this.formatoMiles('$'+(data[_i])[1].toLocaleString('en-us', {minimumFractionDigits: 2})), pendiente: this.formatoMiles('$'+(data[_i])[2].toLocaleString('en-us', {minimumFractionDigits: 2})), exitoso: this.formatoMiles('$'+(data[_i])[3].toLocaleString('en-us', {minimumFractionDigits: 2}))}  
        ];
        this.datosPA= [ ...this.datosP, { unidadNegocio: (data[_i])[0], valorDeuda: (data[_i])[1], pendiente:(data[_i])[2], exitoso: (data[_i])[3]}  
        ];
        this.oficinas= [ ...this.oficinas, { unidadNegocio: (data[_i])[0]}  
        ];
      }

      this.oficinas = this.oficinas.filter(
        (thing, i, arr) => arr.findIndex(t => t.unidadNegocio === thing.unidadNegocio) === i
      );
  ...
  ExportTOExcel()  {
    ...
    /* save to file */
    XLSX.writeFile(wb, 'ArqueoGeneral.xlsx');
  }
  ...
  filtrarReporte(){
    ...
    this.recobrosService.findHerramientasArqueoGeneral().subscribe(data => {     
      ...
      if(this.toppingsOficinas.value!=null){
        if(this.toppingsOficinas.value.length>0){
          for(var _i=0 ; _i<data.length; _i++){
            for(let cod of this.toppingsOficinas.value){
              if((data[_i])[0]== cod){
                if((data[_i])[0]==null){
                ...
                this.datosPA= [ ...this.datosPA, { unidadNegocio: (data[_i])[0], valorDeuda: (data[_i])[1], pendiente:(data[_i])[2], exitoso: (data[_i])[3]}  
                ];        
              }
            }       
          }
