...
import { ListacuentasexcepcionadasDataSource, ListacuentasexcepcionadasItem } from './listacuentasexcepcionadas-datasource';
...
@Component({
  selector: 'app-listacuentasexcepcionadas',
  templateUrl: './listacuentasexcepcionadas.component.html',
  styleUrls: ['./listacuentasexcepcionadas.component.css']
})
export class ListacuentasexcepcionadasComponent implements AfterViewInit, OnInit {
  @ViewChild(MatPaginator, {static: false}) paginator: MatPaginator;
  @ViewChild(MatSort, {static: false}) sort: MatSort;
  @ViewChild(MatTable, {static: false}) table: MatTable<ListacuentasexcepcionadasItem>;
  dataSource: ListacuentasexcepcionadasDataSource;
  ...
  ngOnInit() {
    ...
    this.dataSource = new ListacuentasexcepcionadasDataSource();
    this.dataSource.data=  this.datosP;
    /*
    this.dataSource = new ListacuentasexcepcionadasDataSource();
    this.dataSource.data=  this.datosP;
    this.recobrosService.findCuentasExcepcionadas('', '', '', '', '', '').subscribe(data => {  
      this.datosP=[];    
      console.log(data);
      this.total= 0;
      this.numeroFilas= 0;
      if(data.length!= 0){
        ...
        for(var _i=0 ; _i<data.length; _i++){
          ...
          this.datosP= [ ...this.datosP, { tipoCuenta: (data[_i])[0], numeroCuentaRechazo: (data[_i])[2], codigoTransaccion: (data[_i])[3], 
            fechaDeuda: new Date((data[_i])[5]), tipoIdentificacion: (data[_i])[6], numeroDocumento: (data[_i])[7], 
            ...
            detalleCatalogo: 'Cuenta', detalleExcepcion: (data[_i])[12], codigoOficinaOCeo: (data[_i])[14], 
            nombreOficinaOCeo: (data[_i])[15], resultado: 'Exonerado'}  
          ];
          this.datosExcel= [ ...this.datosExcel, { TipoCuenta: (data[_i])[0], NumeroCuentaRechazo: (data[_i])[2], CodigoTransaccion: (data[_i])[3], 
            FechaDeuda: new Date((data[_i])[5]), TipoIdentificacion: (data[_i])[6], NumeroDocumento: (data[_i])[7], 
            ...
            NombreOficinaOCeo: (data[_i])[15], ExcepcionadoPor: 'Cuenta'}  
          ];
        }
    }
  ...
  ExportTOExcel()  {
    ...
    /* save to file */
    XLSX.writeFile(wb, 'ConsultaCuentasExcepcionadasRecobros.xlsx');
  }
  ...
  filtrarReporte(){  
    ...
    this.recobrosService.findCuentasExcepcionadas(this.pickerInicio, this.pickerFin, this.numDocumento, this.oficina, this.numCuenta, this.codTransaccion).subscribe(data => {     
      this.datosP=[];
      this.datosExcel=[];
      this.dataSource.data = this.datosP;
      this.total= 0;
      ...
      if(data.length!= 0){
        ...
        for(var _i=0 ; _i<data.length; _i++){
          ...
          this.datosExcel= [ ...this.datosExcel, { CodigoTransaccion: (data[_i])[3], FechaRechazo: new Date((data[_i])[16]), FechaDeuda: new Date((data[_i])[5]),
            TipoCuenta: (data[_i])[0], NumeroCuentaRechazo: (data[_i])[2], CodigoOficina: (data[_i])[14], 
            ValorDeuda: (data[_i])[10], ValorPendiente: (data[_i])[11], TipoIdentificacion: (data[_i])[6],
            NumeroDocumento: (data[_i])[7], NombreCliente: (data[_i])[8],  DetalleExcepcion: (data[_i])[12], 
            ExcepcionadoPor: 'Cuenta'}  
          ];
        }
    }
    this.total= this.formatoMiles('$'+this.total.toLocaleString('en-us', {minimumFractionDigits: 2}));    
