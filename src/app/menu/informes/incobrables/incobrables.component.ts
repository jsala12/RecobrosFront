...
@Component({
  selector: 'app-incobrables',
  templateUrl: './incobrables.component.html',
  styleUrls: ['./incobrables.component.css']
})
export class IncobrablesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //-------------------------------------------------------------------------------------------//  
    /*this.recobrosService.findIncobrables('','','').subscribe(data => {
      for(var _i=0 ; _i<data.length; _i++){
        ...
        this.datosP= [ ...this.datosP, { fechaDeuda: fecha, valorDeuda: (data[_i])[1], valorDeudaActual: (data[_i])[2], 
          ...
          codigoOficina: (data[_i])[7], saldoEnCanje: (data[_i])[8], codigoTransaccion: (data[_i])[9], 
          tipoProducto: (data[_i])[10], numeroProducto: (data[_i])[11], tipoIdentidad: (data[_i])[12],
          numeroIdentidad: (data[_i])[13]}
        ];
      };
      this.spinner.hide(); 
    }, error => { 
  ...
  filtrarReporte(){    
    ...
    this.recobrosService.findIncobrables(this.pickerInicio, this.pickerFin, this.codTransaccion).subscribe(data => {     
      this.datosP=[];
      ...
      for(var _i=0 ; _i<data.length; _i++){
        ...
        this.datosP= [ ...this.datosP, { CodigoTransaccion: (data[_i])[9], FechaRechazo: this.fechaRechazo, FechaDeuda: fecha, ValorDeuda: (data[_i])[1], 
          ValorDeudaActual: (data[_i])[2], TipoProducto: (data[_i])[10], NumeroCuentaDeuda: (data[_i])[11], CodigoOficinaRechazo: (data[_i])[7],
          RazonIncobrable: (data[_i])[6], SaldoActualCuenta: (data[_i])[15], EstadoCuenta: (data[_i])[14], EstadoDeuda: this.estadoDeudas, 
