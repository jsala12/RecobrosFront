...
import { CobrosDataSource, CobrosItem } from './cobros-datasource';
...
export class CobrosComponent implements AfterViewInit, OnInit {
  @ViewChild(MatPaginator, {static: false}) paginator: MatPaginator;
  @ViewChild(MatSort, {static: false}) sort: MatSort;
  @ViewChild(MatTable, {static: false}) table: MatTable<CobrosItem>;
  dataSource: CobrosDataSource;
  ...
  ngOnInit() {  
    ...
    //-------------------------------------------------------------------------------------------//  
    this.dataSource = new CobrosDataSource();
    this.dataSource.data=  this.datosP;
    /*this.recobrosService.findCobros('','','').subscribe(data => {
  ...
  ExportTOExcel()  {
    ...
    /* save to file */
    XLSX.writeFile(wb, 'InformeCobros.xlsx');
    this.spinner.hide();
  }
  ...
  filtrarReporte(){    
    ...
    this.recobrosService.findCobros(this.pickerInicio, this.pickerFin, this.codTransaccion).subscribe(data => {     
