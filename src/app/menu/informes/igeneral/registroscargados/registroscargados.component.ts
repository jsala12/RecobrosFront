...
import { RegistroscargadosDataSource, RegistroscargadosItem } from './registroscargados-datasource';
...
@Component({
  selector: 'app-registroscargados',
  templateUrl: './registroscargados.component.html',
  styleUrls: ['./registroscargados.component.css']
})
export class RegistroscargadosComponent implements AfterViewInit, OnInit {
  @ViewChild(MatPaginator, {static: false}) paginator: MatPaginator;
  @ViewChild(MatSort, {static: false}) sort: MatSort;
  @ViewChild(MatTable, {static: false}) table: MatTable<RegistroscargadosItem>;
  @ViewChild(GraficaComponent,{static: false}) private grafica: GraficaComponent;
  dataSource: RegistroscargadosDataSource;
  ...
  ngOnInit() {  
    ...
    //-------------------------------------------------------------------------------------------//  
    this.dataSource = new RegistroscargadosDataSource();
    this.dataSource.data=  this.datosP;/*
    this.recobrosService.findRegistrosCargados('','','').subscribe(data => {
      ...
      for(var _i=0 ; _i<data.length; _i++){
        ...
        this.datosExcel= [ ...this.datosExcel, { CodigoTransaccion: (data[_i])[0], NombreTransaccion: (data[_i])[3], CantidadCargada: (data[_i])[1], 
        ValorCargado: (data[_i])[2]}]; 
  ...
  ExportTOExcel()  {
    ...
    /* save to file */
    XLSX.writeFile(wb, 'InformeRegistrosCargados.xlsx');
    this.spinner.hide();
  }
  ...
  filtrarReporte(){    
    ...
    this.recobrosService.findRegistrosCargados(this.pickerInicio, this.pickerFin, this.codTransaccion).subscribe(data => {     
