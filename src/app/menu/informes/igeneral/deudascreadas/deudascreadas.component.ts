...
import { DeudascreadasDataSource, DeudascreadasItem } from './deudascreadas-datasource';
...
@Component({
  selector: 'app-deudascreadas',
  templateUrl: './deudascreadas.component.html',
  styleUrls: ['./deudascreadas.component.css']
})
export class DeudascreadasComponent implements AfterViewInit, OnInit {
  @ViewChild(MatPaginator, {static: false}) paginator: MatPaginator;
  @ViewChild(MatSort, {static: false}) sort: MatSort;
  @ViewChild(MatTable, {static: false}) table: MatTable<DeudascreadasItem>;
  dataSource: DeudascreadasDataSource;
  ...
  ngOnInit() {
    ...
    //-------------------------------------------------------------------------------------------//  
    this.dataSource = new DeudascreadasDataSource();
    this.dataSource.data=  this.datosP;
    /*
    this.recobrosService.findDeudasCreadas('','','').subscribe(data => {
      ...
      for(var _i=0 ; _i<data.length; _i++){
        ...
        this.datosExcel= [ ...this.datosExcel, { CodigoTransaccion: (data[_i])[0], NombreTransaccion: (data[_i])[3], CantidadDeudasCreadas: (data[_i])[1], 
        ValorDeudasCreadas: (data[_i])[2]} ];
      }
  ...
  ExportTOExcel()  {
    ...
    /* save to file */
    XLSX.writeFile(wb, 'InformeDeudasCreadas.xlsx');
    this.spinner.hide();
  }
  ...
  filtrarReporte(){    
    ...
    this.recobrosService.findDeudasCreadas(this.pickerInicio, this.pickerFin, this.codTransaccion).subscribe(data => {     
