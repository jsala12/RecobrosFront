...
import { IpendientesmesDataSource, IpendientesmesItem } from './ipendientesmes-datasource';
...
@Component({
  selector: 'app-ipendientesmes',
  templateUrl: './ipendientesmes.component.html',
  styleUrls: ['./ipendientesmes.component.css']
})
export class IpendientesmesComponent implements AfterViewInit, OnInit {
  @ViewChild(MatPaginator, {static: false}) paginator: MatPaginator;
  @ViewChild(MatSort, {static: false}) sort: MatSort;
  @ViewChild(MatTable, {static: false}) table: MatTable<IpendientesmesItem>;
  dataSource: IpendientesmesDataSource;
  ...
  ngOnInit() {
    ...
    this.dataSource = new IpendientesmesDataSource();
    this.dataSource.data= this.datosP;
    this.toppingsMeses = new FormControl(); 
    this.toppingsMeses.setValue([]);
    /*
    this.recobrosService.findPendienteCobrarMes().subscribe(data => {
  ...
  filtrarTabla(){
    ...
    this.recobrosService.findPendienteCobrarMes().subscribe(data => {
      ...
      this.spinner.hide(); 
      this.alertService.warning("No se pudo cargar la información de pendientes por mes: "+error);
      this.dataSource.data= [];
    });
  }
  ...
  ExportTOExcel()  {
    ...
    /* save to file */
    XLSX.writeFile(wb, 'InformePendientesMes.xlsx');
    this.spinner.hide();
  }
}
