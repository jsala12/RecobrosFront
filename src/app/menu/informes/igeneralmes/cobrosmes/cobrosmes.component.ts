...
import { CobrosmesDataSource, CobrosmesItem } from './cobrosmes-datasource';
...
@Component({
  selector: 'app-cobrosmes',
  templateUrl: './cobrosmes.component.html',
  styleUrls: ['./cobrosmes.component.css']
})
export class CobrosmesComponent implements AfterViewInit, OnInit {
  @ViewChild(MatPaginator, {static: false}) paginator: MatPaginator;
  @ViewChild(MatSort, {static: false}) sort: MatSort;
  @ViewChild(MatTable, {static: false}) table: MatTable<CobrosmesItem>;
  dataSource: CobrosmesDataSource;
  ...
  ngOnInit() {
    ...
    this.dataSource = new CobrosmesDataSource();
    this.dataSource.data= this.datosP;
    this.toppingsMeses = new FormControl(); 
    this.toppingsMeses.setValue([]);
    /*
    this.recobrosService.findCobrosCreadosMes().subscribe(data => {
      ...
      for (var _i = 0; _i < data.length; _i++) {
        ...
        this.datosP = [...this.datosP, {
          anoMes: (data[_i])[0], cantidad: this.formatoMiles((data[_i])[1].toLocaleString('en-us')),
          valor: this.formatoMiles('$' + (data[_i])[2].toLocaleString('en-us', { minimumFractionDigits: 2 }))}];

        this.datosExcel = [...this.datosExcel, {
          Periodo: (data[_i])[0], Cantidad:(data[_i])[1], Valor: (data[_i])[2]}];
  ...
  ExportTOExcel()  {
    ...
    /* save to file */
    XLSX.writeFile(wb, 'InformeCobrosCreadosMes.xlsx');
    this.spinner.hide();
  }
}
