...
@Component({
  selector: 'app-idescargablependientes',
  templateUrl: './idescargablependientes.component.html',
  styleUrls: ['./idescargablependientes.component.css']
})
export class IdescargablependientesComponent implements OnInit {
  ...
  filtrarReporte(){    
    ...
    this.recobrosService.findTodosPendientes(this.pickerInicio, this.pickerFin, this.codTransaccion).subscribe(data => {     
      this.datosP=[];
      ...
      for(var _i=0 ; _i<data.length; _i++){
        ...
        let fecha = new Date((data[_i])[0])
        fecha.setMinutes(fecha.getMinutes() + fecha.getTimezoneOffset());
        this.datosP= [ ...this.datosP, { CodigoTransaccion: (data[_i])[9], FechaRechazo: this.fechaRechazo, FechaDeuda: fecha, ValorDeuda: (data[_i])[1], TipoProducto: (data[_i])[10],
          NumeroCuentaDeuda: (data[_i])[11], CodigoOficinaRechazo: (data[_i])[7], EstadoDeuda: this.estadoDeudas, ValorCobrado: (data[_i])[1]-(data[_i])[2], 
          SaldoCuentaDeuda: (data[_i])[15], EstadoCuentaDeuda: (data[_i])[14], ValorDeudaActual: (data[_i])[2], TipoIdentidadCliente: (data[_i])[12], 
          NumeroIdentidadCliente: (data[_i])[13], Incobrable: this.incobrable, RazonIncobrable: (data[_i])[6]}
        ];
      };      
      this.spinner.hide();
    }, error => { 
